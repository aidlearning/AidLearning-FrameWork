...
- New Gui Design Interface (Wizard), now GUI design does not require you to write code generation, you can use touch and drag to generate Gui interface, the system will automatically generate GUI code, you only need to focus on the core processing code.
3.  Redesign the example program and use the new Gui development method.
4.  Because of the introduction of touch drag-and-drop programming, the development code now adopts the method of separating the interface from the personal core processing code. The interface code is generated by the system, and the core processing code system also generates if, you can focus on the part you really want to deal with.
